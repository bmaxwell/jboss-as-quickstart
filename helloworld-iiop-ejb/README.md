QUICKSTART NAME: Brief Description of the Quickstart
======================================================
Author: Brad Maxwell <bmaxwell@redhat.com>
Level: Beginner
Technologies: EJB 3.1, IIOP
Summary: JDK client application calls EJB via IIOP
Prerequisites: none
Target Product: EAP
...
This Quickstart demostrates how to expose an EJB session bean via IIOP and an external JDK client that calls the EJB remotely.  It demostrates EJB 3.1 and IIOP.
When the quickstart is run it deploys a jar which starts a Stateless EJB exposing an IIOP interface running in the JBoss EAP server instance.  When the client module is run it looks up the EJB running in the JBoss EAP server instance and invokes its `hello` method passing it the name `Client` and the EJB will respond using the name passed in which will be `Hello Client, my name is JBoss`

org.jboss.as.quickstarts.helloworld.iiop.client.JDKClient

This is a standalone remote client that is running from the command line.
The client uses the JDK's `com.sun.jndi.cosnaming.CNCtxFactory` initial context factory to look up the EJB IIOP's interface in JBoss EAP6.  It runs with the property `com.sun.CORBA.ORBUseDynamicStub` set to true to use dynamic stubs so that the client does not have to generate stubs before hand.
After it pulls the stub from the remote JBoss EAP's naming server it uses the EJB's home interface and invokes create to get the remote interface and then calls the HelloWorld EJB and prints the response.


org.jboss.as.quickstarts.helloworld.iiop.ejb.HelloRemoteHome

- This is the EJB's remote home interface, which has one method create which returns the Remote interface (HelloRemote)

org.jboss.as.quickstarts.helloworld.iiop.ejb.HelloRemote

- This is the EJB's remote interface, which has one method hello which takes a string and returns a string

org.jboss.as.quickstarts.helloworld.iiop.ejb.HelloSessionBean

- This is the EJB implementation, which uses EJB 3.1 annotations.  It implements the hello method.

jboss-ejb3.xml

- This is the JBoss specific deployment descriptor which is used for JBoss specific EJB configurations.  It is used to expose the EJB via IIOP.  In this Quickstart, it uses * for the ejb-name which means all EJB's in this jar deployment should be exposed via iiop.  To only have particular EJBs exposed via IIOP, you would simply specify the EJB's names you want exposed.  The EJB name is specified in via the name in annotation @Stateless, @Stateful or in the ejb-jar.xml when defining the EJB.  If you do not define an EJB name it defaults to the name of the EJB implementation class name such as HelloSessionBean in this Quickstart.
...
4. This will deploy `ejb/target/jboss-as-helloworld-iiop-ejb-ejb.jar`) to the running instance of the server.
5. Type this command to run the client application which will call the ejb running in JBoss 

        mvn --projects client exec:exec

6. The console will log:

        remote.hello('Client') => Hello Client, my name is JBoss
